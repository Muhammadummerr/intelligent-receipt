...
def load_entities(path: str) -> EntityGT:
    """
    Load entity annotations from JSON (or JSON-like .txt) files.
    Returns normalized EntityGT dataclass.
    """
    with open(path, "r", encoding="utf-8") as f:
        try:
            j = json.load(f)
        except Exception:
            # fallback: parse line-by-line JSON-like file
            lines = f.readlines()
            j = {}
            for line in lines:
                if ":" in line:
...
def is_total_amount(line: str) -> Optional[float]:
    ...
    if re.search(r"\b(total|grand\s*total|amount\s*due|cash)\b", s, flags=re.I) \
       or re.search(r"\b\d+\.\d{2}\b", s) \
       or re.search(r"\bRM\s*\d", line, flags=re.I):
...
# ----------------------------------------------------------------------
# Main Alignment Function (new)
# ----------------------------------------------------------------------
def assign_lines_to_fields(lines: List[BoxLine], gt: EntityGT) -> Dict[int, str]:
    """
    Robust alignment between OCR text lines and ground-truth entity values.
    Matches lines based on fuzzy similarity to gt.company, gt.date, etc.
    Returns mapping {index -> FIELD_NAME}.
    """
    mapping: Dict[int, str] = {}
    ro_lines = sort_reading_order(lines)

    gt_dict = {
        "COMPANY": gt.company.lower(),
        "DATE": gt.date.lower(),
        "ADDRESS": gt.address.lower(),
        "TOTAL": gt.total.lower()
    }

    for i, li in enumerate(ro_lines):
        text = normalize_spaces(li.text).lower()
        if not text:
            continue

        best_field, best_score = None, 0.0

        # Direct and fuzzy matching with ground truth
        for field, gold in gt_dict.items():
            if not gold:
                continue
            ...
            # Fuzzy similarity
            score = fuzz.token_set_ratio(text, gold)
            if score > best_score:
        ...
        # Heuristic fallback (only if fuzzy failed)
        if is_date_like(text):
            mapping[i] = "DATE"
        elif is_total_amount(text):
            mapping[i] = "TOTAL"
        elif re.search(r"JALAN|TAMAN|SELANGOR|KUALA\s*LUMPUR|MALAYSIA", text, re.I):
            mapping[i] = "ADDRESS"
        elif i < 3:  # top of page heuristic for company name
            mapping[i] = "COMPANY"
